...
function onclick( event ) {
	...
	//var intersects = raycaster.intersectObjects(sphere2, true); //array
	var intersects = raycaster.intersectObjects( scene.children );
	for ( var i = 0; i < intersects.length; i++ ) {

		if (intersects[ i ].object.position.x !=0 || intersects[ i ].object.position.y !=0 || intersects[ i ].object.position.z !=0){
			console.log(intersects[ i ].object.position);
			if(intersects[ i ].object.name == "sfera0"){
				totPath[totPath.length-1]= currentPosition;
				totOrient[totOrient.length-1]= currentOrientation;
				//newPathPosition=[];
				//totPath.push(currentPosition);
				totPath.push([0.4,1.1,-1]);
				totPath.push([0.5,1.03,-2]);
				totPath.push([0.4,1.1,-1]);
				//console.log(newPath[0],newPath[1]);
				//totOrient=[];
				//totOrient.push(currentOrientation);
				totOrient.push([0.1,-0.5,-1]);
				totOrient.push([0.2,-0.5,-0.5]);
				totOrient.push([0.1,-0.5,-1]);
			}
			else if(intersects[ i ].object.name == "sfera1"){
				totPath[totPath.length-1]= currentPosition;
				totOrient[totOrient.length-1]= currentOrientation;
				//newPathPosition=[];
				//totPath.push(currentPosition);
				totPath.push([-1,2,-1.3]);
				totPath.push([-0.5,1.74,-2]);
				totPath.push([-1,2,-1.3]);
				//console.log(newPath[0],newPath[1]);
				//newPathOrientation=[];
				//totOrient.push(currentOrientation);
				totOrient.push([0,0,-1]);
				totOrient.push([1,-0.5,0]);
				totOrient.push([0,0,-1]);
			}
			else if(intersects[ i ].object.name == "sfera2"){
				if(totPath[totPath.length-1][0]!=-0.5 ||totPath[totPath.length-1][1]!=1.31 ||totPath[totPath.length-1][2]!=2 ){
					totPath[totPath.length-1]= currentPosition;
					totOrient[totOrient.length-1]= currentOrientation;
					totPath.push([-0.5,1.31,2]);
					totOrient.push([-1,-1,0]);
				}
			}
			else if(intersects[ i ].object.name == "sfera3"){
				//newPathPosition=[];
				//totPath.push(currentPosition);
				if(totPath[totPath.length-1][0]!=-2 || totPath[totPath.length-1][1]!=1.5 ||totPath[totPath.length-1][2]!=0 ){
					totPath[totPath.length-1]= currentPosition;
					totOrient[totOrient.length-1]= currentOrientation;
					totPath.push([-2,1.5,0]);
					totOrient.push([-0.5,0,0]);
				}
			}
			else if(intersects[ i ].object.name == "sfera4"){
				//newPathPosition=[];
				//totPath.push(currentPosition);
				if(totPath[totPath.length-1][0]!=1.5 || totPath[totPath.length-1][1]!=1.8 || totPath[totPath.length-1][2]!=0 ){
					totPath[totPath.length-1]= currentPosition;
					totOrient[totOrient.length-1]= currentOrientation;
					totPath.push([1.5,1.8,0]);
					totOrient.push([0.5,0,0]);
				}
			}
			//totPath=newPathPosition;
			//totOrient=newPathOrientation;
			kin= true;
			//pathNumber=0;
			done= false;
			console.log(totPath)
...
function init() {
	...
	sphere0.name="sfera0";
	sphere0.position.set( 0.5,1.03,-2 );
	scene.add( sphere0 );

	sphere1 = new THREE.Mesh( geometry, material );
	sphere1.castShadow = true;
	sphere1.name="sfera1";
	sphere1.position.set( -0.5,1.74,-2 );
	scene.add( sphere1 );									// ripiano in alto

	sphere2 = new THREE.Mesh( geometry, material );
	sphere2.castShadow = true;
	sphere2.name="sfera2";
	sphere2.position.set( -0.5,1.31,2 );
	scene.add( sphere2 );									// sopra il tavolo

	sphere3 = new THREE.Mesh( geometry, material );
	sphere3.castShadow = true;
	sphere3.name="sfera3";
	sphere3.position.set( -2,1.5,0 );
	scene.add( sphere3 );									// dietro

	sphere4 = new THREE.Mesh( geometry, material );
	sphere4.castShadow = true;
	sphere4.name="sfera4";
	sphere4.position.set( 1.5,1.8,0 );
	scene.add( sphere4 );									// davanti
...
function uptadeMovement(t){
	...
	if (pathNumber< totPath.length-1){
		...
		console.log("K")
		kin=false;
		var poseAtTime= [currentPosition,currentOrientation];
	}
	var positionAtTime= poseAtTime[0];
	currentPosition=positionAtTime;
	var orientationAtTime= poseAtTime[1];
	currentOrientation= orientationAtTime;
...
// GUI
function directKinematics(){
	var beta= baseArm.rotation.y;
	var ang1= arms[0].rotation.x;
	var ang2= arms[1].rotation.x;
	var ang3= arms[2].rotation.x;
	currentPosition= [Math.sin(beta)*(Math.sin(ang1) + Math.sin(ang1+ang2)+ Math.sin(ang1+ang2+ang3)),
						1 + Math.cos(ang1)+ Math.cos(ang1+ang2)+ Math.cos(ang1+ang2+ang3),
						 Math.cos(beta)*(Math.sin(ang1) + Math.sin(ang1+ang2)+ Math.sin(ang1+ang2+ang3))]
	currentOrientation=[Math.sin(beta)*Math.sin(ang1+ang2+ang3),
						Math.cos(ang1+ang2+ang3),
						Math.cos(beta)*(Math.sin(ang1+ang2+ang3))]
}
...
function updateDatGui() {
	...
	console.log(9);
	folder.add(baseArm.rotation, 'y', -Math.PI, +Math.PI).name('rotation_Y').onChange(function() {
			//console.log(32);
			directKinematics();
		});
